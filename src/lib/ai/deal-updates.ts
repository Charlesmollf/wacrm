...
export const DEAL_EXTRACTION_INSTRUCTIONS =
  ...
  'CARRITO (INVISIBLE, OBLIGATORIA SIEMPRE QUE HAYA PEDIDO EN CURSO): agrega ademas esta segunda marca al final, y es la UNICA fuente del pedido: el sistema ya no lee ningun campo combo=, lee esta marca. ' +
  ...
  'COMBOS, CONTENIDO FIJO: cada combo trae SIEMPRE las mismas variedades (Colosos de America = Pacamara+Maracaturra+Maragogipe; Intensa Dulzura = Pacamara+Catuai+Anaerobico; Mitico Coban = Bourbon+Catuai+Caturra Roja; Africa Mia = Gesha+Kenia SL28; Procesos Secretos = Anaerobico+Peaberry), el cliente no las elige ni las cambia. NUNCA anotes esas variedades como productos aparte en la marca CARRITO (eso duplica el pedido y lo cobra de mas): el combo va como UN solo producto de la lista, por su nombre. Si dentro de un combo cada bolsa lleva una molienda distinta (ahi es cuando usas molienda=Mixto), acláralo entre parentesis PEGADO al nombre del combo, en la MISMA marca CARRITO: [[CARRITO: 1 Colosos de America (Pacamara grano, Maracaturra grano, Maragogipe molido)]]. Esa aclaracion se guarda tal cual para que el equipo sepa que empacar; no la repartas en productos sueltos. ' +
