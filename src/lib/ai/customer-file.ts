...
const REQUERIDOS = ['nombre', 'correo', 'direccion', 'grano o molido', 'producto', 'forma de pago'] as const
...
export async function buildCustomerFile(
  ...
  try {
    ...
    const context =
      ...
      `   📍 Direccion exacta\n` +
      `   ⚙️ ¿Grano o molido?\n` +
      `   💳 Forma de pago: 1) Link de pago (solo VISA), 2) Transferencia BAM, 3) Contra entrega\n` +
      `   Despues, en cada respuesta, reconoce en una linea y pide UNICAMENTE lo que siga faltando.\n` +
      `   Ej.: "Anotado 😊 Solo me faltaria su correo." El telefono ya lo tenemos por WhatsApp: no lo pidas.\n` +
      ...
      `   y NO sirve para rotular la guia de envio: pidelo como "👤 Nombre completo" DENTRO del mismo mensaje de datos\n` +
      `   (regla 2), no en un mensaje aparte. Sin nombre confirmado no mandes el resumen ni cierres el pedido.\n` +
